...
cam_params_dict = {
    "TUM/rgbd_dataset_freiburg1_desk": [
        517.306408,
        516.469215,
        318.643040,
        255.313989,
...
def get_K_in_K_out(cam_params, h, w):
    ...
    K_out = cv2.getOptimalNewCameraMatrix(
        K_in, np.array(cam_params[4:]), (w, h), 1, (w, h), True
    )[0]
    K_out[0, 0] = K_out[1, 1] = (K_out[0, 0] + K_out[1, 1]) / 2
    return K_in, K_out


def rectify_and_mask(
    image, rectify_map, initial_mask, threshold=250, zero_invalid=True, add_alpha=True
):
    dst = cv2.remap(
        image, rectify_map, None, cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT
    )
...
if __name__ == "__main__":
    ...
    for scene, cam_params in cam_params_dict.items():
        ...
        rectify_map = cv2.initUndistortRectifyMap(
            K_in, np.array(cam_params[4:]), None, K_out, (w, h), cv2.CV_32FC2
        )[0]
