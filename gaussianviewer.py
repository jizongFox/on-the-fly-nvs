...
class GaussianViewer(Viewer):
    ...
    def reset_intrinsics(self, view):
        ...
        camera.res_x = (
            self.scene_model.width // 2
            if view == "top_view"
            else self.scene_model.width
        )
        camera.fov_x = self.scene_model.FoVx
        camera.res_y = (
            self.scene_model.height // 2
            if view == "top_view"
            else self.scene_model.height
        )
    ...
    def step(self):
        ...
        if self.snap_mode.value in [SnapMode.keyframe, SnapMode.last]:
            ...
            keyframe_id = (
                self.keyframe_id if self.snap_mode.value == SnapMode.keyframe else -1
            )
            point_viewmatrix = self.scene_model.keyframes[keyframe_id].get_Rt()
            self.updated_pose = (
                torch.linalg.inv(point_viewmatrix.detach()).cpu().numpy()
            )
        else:
            if self.altitude_control:
                camera_position = torch.tensor(
                    self.point_view_camera.origin, dtype=torch.float32
                ).cuda()
                n_closest = 4
                closest_keyframes = self.scene_model.get_closest_keyframe(
                    camera_position, n_closest
                )
                mean_closest_altitude = (
                    torch.stack([kf.approx_centre for kf in closest_keyframes]).sum(
                        axis=0
                    )
                    / n_closest
                )[1]
                ...
                if abs(mean_closest_altitude - self.point_view_camera.origin[1]) > 1e-4:
                    ...
                    to_world[1, 3] += (
                        (1.0 - self.altitude_smoothing)
                        * self.point_view_camera.speed
                        * dist
                    )
            ...
            if self.snap_to_closest:
                ...
                closest_keyframe = self.scene_model.get_closest_keyframe(
                    camera_position
                )[0]
                keyframe_pose = (
                    torch.linalg.inv(closest_keyframe.get_Rt()).detach().cpu().numpy()
                )
        ...
        for view in ["point_view", "top_view"]:
            ...
            # Draw Gaussians
            if (view == "point_view" and self.render_mode() in ["Splats", "Depth"]) or (
                view == "top_view" and self.show_top_view
            ):
                width = camera.res_x
                height = camera.res_y
                viewmatrix = (
                    torch.tensor(camera.to_camera, dtype=torch.float32)
                    .cuda()
                    .transpose(0, 1)
                )
                ...
                if self.render_mode() == "Splats":
                    ...
                    image = cv2.cvtColor(
                        cv2.applyColorMap(image, cv2.COLORMAP_INFERNO),
                        cv2.COLOR_BGR2RGB,
                    )
                ...
                # Draw overlays
                if (
                    self.draw_anchors[view]
                    or self.draw_poses[view]
                    or self.draw_gt_poses[view]
                    or view == "top_view"
                ):
                    ...
                    common_opt = (
                        ...
                        viewmatrix,
                        camera.fov_x,
                        self.pose_sizes[view],
                        self.scene_model.width,
                        self.scene_model.height,
                    )
                    if self.draw_gt_poses[view]:
                        image = draw_poses(
                            *common_opt,
                            self.scene_model.get_gt_Rts(True),
                            self.scene_model.gt_f,
                            (255, 0, 0),
                        )
                    if self.draw_poses[view]:
                        Rts = self.scene_model.get_Rts()
                        image = draw_poses(
                            *common_opt, Rts, self.scene_model.f, (255, 255, 255)
                        )
                    if self.draw_anchors[view]:
                        image = draw_anchors(
                            ...
                            viewmatrix,
                            camera.fov_x,
                            self.anchor_sizes[view],
                            self.scene_model.anchors,
                            self.scene_model.anchor_weights,
                        )
                    if view == "top_view":
                        point_viewmatrix = torch.tensor(
                            self.point_view_camera.to_camera, dtype=torch.float32
                        ).cuda()[None]
                        image = draw_poses(
                            *common_opt,
                            point_viewmatrix,
    ...
    def show_gui(self):
        ...
        with imgui_ctx.begin(f"Point View Settings"):
            ...
            _, self.render_mode_id = imgui.list_box(
                "Render Mode", self.render_mode_id, render_modes
            )

            imgui.separator_text("Render Settings")
            if self.render_mode() in ["Splats", "Depth"]:
                _, self.scaling_factor["point_view"] = imgui.slider_float(
                    "Scaling Factor", self.scaling_factor["point_view"], 1e-2, 1
                )
                _, self.draw_poses["point_view"] = imgui.checkbox(
                    "Draw Poses", self.draw_poses["point_view"]
                )
                _, self.draw_gt_poses["point_view"] = imgui.checkbox(
                    "Draw GT Poses", self.draw_gt_poses["point_view"]
                )
                if self.draw_poses["point_view"] or self.draw_gt_poses["point_view"]:
                    _, self.pose_sizes["point_view"] = imgui.drag_float(
                        "Pose Sizes",
                        self.pose_sizes["point_view"],
                        0.01,
                        0,
                        1e8,
                        "%.2f",
                    )
                _, self.draw_anchors["point_view"] = imgui.checkbox(
                    "Draw Anchors", self.draw_anchors["point_view"]
                )
                if self.draw_anchors["point_view"]:
                    _, self.anchor_sizes["point_view"] = imgui.drag_float(
                        "Anchor Sizes",
                        self.anchor_sizes["point_view"],
                        0.01,
                        0,
                        1e8,
                        "%.2f",
            ...
            if self.snap_mode.value == SnapMode.keyframe:
                ...
                imgui.same_line()
                _, self.keyframe_id = imgui.slider_int(
                    "##", self.keyframe_id, 0, max(0, self.num_keyframes - 1)
                )
                imgui.same_line()
                self.next_keyframe = imgui.button("+")
            imgui.separator()
            self.point_view_camera.show_gui()
            _, self.altitude_control = imgui.checkbox(
                "Altitude Control", self.altitude_control
            )
            if self.altitude_control:
                _, self.altitude_smoothing = imgui.slider_float(
                    "smoothing", self.altitude_smoothing, 0.9, 0.9999
                )
        ...
        with imgui_ctx.begin("Top View Settings"):
            ...
            if self.show_top_view:
                imgui.separator_text("Render Settings")
                _, self.scaling_factor["top_view"] = imgui.slider_float(
                    "Scaling Factor", self.scaling_factor["top_view"], 1e-3, 1e-2
                )
                _, self.draw_poses["top_view"] = imgui.checkbox(
                    "Draw Poses", self.draw_poses["top_view"]
                )
                _, self.draw_gt_poses["top_view"] = imgui.checkbox(
                    "Draw GT Poses", self.draw_gt_poses["top_view"]
                )
                if self.draw_poses["top_view"] or self.draw_gt_poses["top_view"]:
                    _, self.pose_sizes["top_view"] = imgui.drag_float(
                        "Pose Sizes", self.pose_sizes["top_view"], 0.01, 0, 1e8, "%.2f"
                    )
                _, self.draw_anchors["top_view"] = imgui.checkbox(
                    "Draw Anchors", self.draw_anchors["top_view"]
                )
                if self.draw_anchors["top_view"]:
                    _, self.anchor_sizes["top_view"] = imgui.drag_float(
                        "Anchor Sizes",
                        self.anchor_sizes["top_view"],
                        0.01,
                        0,
                        1e8,
                        "%.2f",
                ...
                self.reset_intrinsics_flag["top_view"] = imgui.button(
                    "Reset Intrinsics"
                )
